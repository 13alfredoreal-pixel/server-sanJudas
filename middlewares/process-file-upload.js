  ...
  try {
    ...
    if (!type || !type.mime.startsWith('image/')) {
      return res.status(400).json({
        success: false,
        message:
          'Firma de archivo inválida: La imagen proporcionada no tiene un formato válido o es maliciosa.',
      });
    }
    ...
    console.log(
      `[Cloudinary] Iniciando subida de: ${req.file.originalname} (${req.file.size} bytes)`,
    );
    ...
    if (url.includes('/user') || url.includes('/auth')) {
      ...
      console.log(
        `[Cloudinary] Salto de subida: La ruta ${url} no está autorizada para almacenamiento en nube.`,
      );
      return next();
    }
