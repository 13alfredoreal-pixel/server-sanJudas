  ...
  try {
    ...
    if (req.fileRelativePath) {
      ...
      if (currentUser && currentUser.profilePicturePublicId) {
        try {
          console.log(
            `[UpdateProfile] Intentando eliminar imagen antigua: ${currentUser.profilePicturePublicId}`,
          );
          await cloudinary.uploader.destroy(currentUser.profilePicturePublicId);
        } catch (trashError) {
          console.error(
            `[UpdateProfile] Error no crítico al borrar imagen antigua:`,
            trashError.message,
          );
        }
      }
  ...
  try {
    ...
    if (req.uid) {
      await logAdminAction(
        req.uid,
        'PROMOTE_USER',
        `Otorgado rol de administrador a: ${user.username}`,
        req.ip,
      );
    }
  ...
  try {
    ...
    if (requester.role === 'ADMIN_ROLE' && requesterId !== id) {
      await logAdminAction(
        requesterId,
        'DELETE_USER',
        `Usuario eliminado: ${userToDelete.username}`,
        req.ip,
      );
    }
...
export const updateReadingProgress = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: 'Error al guardar progreso', error: error.message });
  }
};
